...
function main() {
    ...
    //// start and end date range for the search query data
    //// format is yyy-mm-dd
    
    var currencySymbol = "R$";
    //// the currency symbol used, e.g. reais, dollars, pounds

    var campaignNameContains = "";
    //// use this if you want to look at specifc campaigns (i.e. campaign containing "brandX")
    //// leave as ""if not wanted

    var campaignNameDoesNotContain = "";
    //// use this to exclude campaigns conaings specific names
    //// leave as "" if not wanted

    var ignorePausedCampaign = true;
    //// set true to currently active campaigns and false to paused campaigns

    var ignorePausedAdGroups = true;
    //// set true to currently active groups and false to paused groups

    var checkNegatives = true;
    //// set true to remove queries that would be excluded by negative defined keywords
    
    var spreadsheetUrl = "https://docs.google.com/sssssssssss"
    //// The URL of the Google sheets the results will be put into

    var minNGramLength = 1;
    var maxNGramLength = 2;
    /// the min and max length of N-Grams to be checked (e.g. 1 min and 2 max will define phrases of 1, 2 and 3 words to be checked)
    ...
    if (checkNegatives) {
      ...
      var keywordReport = AdWordsApp.report(
        ...
        while (campaignNegativeRows.hasNext()) {
          ...
          if (negativesByCampaigncampaignNegativeRow["CampaignId"]] == undefined) {
              negativesByCampaign[campaignNegativeRow["CampaignId"]] = [[campaignNegativeRow["Criteria"].toLowerCase(),
                  campaignNegativeRow["KeywordMatchType"].toLowerCase()]];
          } else {
              negativesByCampaign[campaignNegativeRow["CampaignId"]].push([campaignNegativeRow["Criteria"].toLowerCase(),
                  campaignNegativeRow["KeywordMatchType"].toLowerCase()]);
          }
        ...
        // Find which campaigns use shared negative keyword sets
            ...
            // Collect the negative keyword text from the sets,  and record it as a campaign level negative in the campaigns that use the set
            ...
            while (sharedSetRows.hasNext()) {
              ...
              if (sharedSetCampaigns[setName] !== undefined) {
                for (var i=0; i<sharedSetCampaigns[setName].length; i++){
                  var campaignId = sharedSetCampigns[setName][i];
        ...
        //////////////////////////////////////////////////////////////////////////////
        //// Define the statistic data  to download and calculate and their format
        ...
        //////////////////////////////////////////////////////////////////////////////
        //// Go through the search query report, remove searches already excluded by negatives and record the performance of each word in
        ////each remaining query
        ...
        // Organize the ad group level stats into an array for output
        for (var n=minNGramLength; n<maxNGramLength+1; n++) {
                      ...
                      // skips nGrams under the thresholds
                        ...
                        for (var s=0; s<calculatedStats.length; s++) {
                            ...
                            if (adGroupNGrams[campaign][adGroup][n][nGram][divisor] > 0) {
                                printline.push(adGroupNGrams[campaign][adGroup][n][nGram][multiplier] / adGroupNGrams[campaign]
                                [adGroup][n][nGram][divisor]);
                            } else {
                                printline.push("-");
                            } 
                        }
        ...
        // Organize the campaign level stats into an array for output
         for (var n=minNGramLength; n<maxNGramLength+1; n++) {
        ...
        // Organize the account level stats into an array for output
        for (var n=minNGramLength; n<maxNGramLength+1; n++) {
        ...
        // Organize the word count analysis into an array for output
        for (var i = 1; i<8; i++) {
            if (i <7) {
                var wordLength = i;
            } else {
      ...
      for (var i=0; i<5; i++) {
          try {
              ...
              if (formatArray.length == 0) {
                  ...
                  sheet.getRange("R" + (lastRow + 2f (doneNGrams.indexOf(currentNGram) < 0) {
                  319}) + "C1:R" + (lastRow+outputArray.length) + "C" + outputArray[0].length).sort(sortByColumns);
              }
